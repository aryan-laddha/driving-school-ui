// src/components/VehicleScheduleModal.jsx
...
function VehicleScheduleModal({ vehicle, onClose }) {
    const [schedules, setSchedules] = useState([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);
    
    // Manage the date being viewed (initialized to today)
    const [currentDate, setCurrentDate] = useState(dateToIsoString(new Date()));

    // Fetch schedules based on vehicle number
    const fetchVehicleSchedules = useCallback(async (dateString) => {
        setLoading(true);
        setError(null);
        ...
        try {
            ...
            console.error('Vehicle schedule fetch error:', err);
            setError(`Failed to load schedules: ${err.message}`);
            toast.error(`Error loading schedule for ${vehicle.vehicleNumber}.`);
        } finally {
            setLoading(false);
        }
    ...
    useEffect(() => {
        fetchVehicleSchedules(currentDate);
    }, [currentDate, fetchVehicleSchedules]);
    ...
    return (
        // FIX: Removed bg-gray-900 bg-opacity-50 to keep the background visible
            ...
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden">
                ...
                <div className="p-5 border-b flex justify-between items-center">
                    <h3 className="text-xl font-bold text-gray-800 flex items-center">
                        <CalendarDaysIcon className="w-5 h-5 mr-2 text-indigo-600" /> Schedule for {vehicle.vehicleName} ({vehicle.vehicleNumber})
                    </h3>
...
export default VehicleScheduleModal;